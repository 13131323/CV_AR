...
class DepthEstimator:
    ...
    def get_depth_map(self, frame):
        ...
        # 4. 시각화용 0~255 정규화 맵 생성 (가까운 것이 밝게 나오도록 반전)
        d_min, d_max = raw_depth.min(), raw_depth.max()
        if d_max - d_min > 1e-5:
            # 원본: (raw_depth - d_min) / (d_max - d_min) * 255.0
            # 반전: 255.0 - 원본
            visual_depth = 255.0 - ((raw_depth - d_min) / (d_max - d_min) * 255.0)
        else:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # 화면 팝업 출력
        cv2.imshow("CV_AR - Depth Anything V2 (Metric)", color_depth)
