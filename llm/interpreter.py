...
2. 'detected_class'도 정확하지 않을 가능성이 높으므로 보정을 통해 값을 수정하라.
3. (중요) `social_state`는 이 객체가 누군가에 의해 점유되어 있는지를 나타낸다. 누군가 손에 쥐고 있거나 사용 중이면 `held_by_user` 또는 `in_use_by_other`로 설정하라.
4. (중요) 사람(`is_person`=true)이거나 `social_state`가 `held_by_user` 또는 `in_use_by_other`인 경우, 아바타가 접근하는 것은 안전하지 않으므로 `planner_directives.action_policy`를 반드시 `IGNORE` 또는 `OBSERVE_ONLY`로 강제하고 `is_safe_to_approach`를 false로 설정하라.
5. `environment_relative`가 `on_floor` 또는 `on_surface`이고 `social_state`가 `available`인 경우에만 `action_policy`를 `APPROACH_AND_INTERACT`로 설정할 수 있다.
6. 추론 이유(`reasoning`)는 반드시 한국어로 1문장 내외로 간결하게 작성하라.
7. 응답 형식은 강제된 JSON 스키마를 완벽히 준수해야 하며, 입력된 객체 배열의 순서와 1:1로 대응해야 한다.
8. `semantic_state.affordances`는 반드시 아래 허용 태그 중 하나 이상을 리스트로 선택하라. 목록에 없는 문자열은 절대 만들지 마라.
