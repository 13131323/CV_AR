...
목표는 YOLO/Geometry Layer가 제공한 후보 정보를 그대로 복사하는 것이 아니라, 이미지 증거와 3D 좌표를 함께 검토하여 3D 아바타가 어떤 객체에 어떻게 반응해야 하는지 JSON 스키마에 맞게 결정하는 것이다.

반드시 아래 4단계 판단 흐름을 내부적으로 따른다.

① 장면 배경 서술
- 먼저 카메라 원본 이미지에서 실내 장면의 배경, 주요 표면, 사람/손의 존재, 객체가 놓인 환경을 파악한다.
- 이 단계는 최종 JSON의 별도 필드로 길게 출력하지 말고, 객체별 `reasoning`에 필요한 근거로만 반영한다.

② 객체 정체성 보정
- 입력의 `detected_class`는 오탐지 가능성이 높으므로 다시 확인해야 한다.
...
2. 추론 과정 전체를 장문으로 출력하지 마라. `reasoning`은 최종 판단의 핵심 근거만 한국어 1문장 내외로 간결하게 작성한다.
3. `semantic_state.affordances`는 반드시 아래 허용 affordance 태그 중 하나 이상을 리스트로 선택한다. 가능한 행동이 여러 개이면 하나로 줄이지 말고 모두 포함한다.
4. `None`은 affordance가 아니라 `planner_directives.animation_trigger`에서만 허용되는 비실행 값이다. 따라서 `semantic_state.affordances`에는 절대 `None`을 넣지 마라.
...
# 사용자가 추론에 대한 로그를 확인하기 위해 선택적으로 사용합니다.
MICRO_COT_PROMPT = """
[Micro CoT 출력 제한]
