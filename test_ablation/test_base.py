"""Ablation baseline: 전체 파이프라인 기준값 수집.

전체 파이프라인을 그대로 실행한다.

생성 파일 3쌍:
1. 전체 파이프라인 시간
   - base_time_res_XXX.csv/txt
   - 전체 파이프라인 시간 30개 + 통계(mean/std/median/Q1/Q3/IQR/outlier)

2. FAST_STREAM 데이터
   - base_fast_stream_res_XXX.csv/txt
   - Vision/Geometry 이후 Unity로 빠르게 보내는 좌표 데이터 30개

3. VLM 데이터
   - base_vlm_res_XXX.csv/txt
   - VLM 추론 결과 JSON 30개
"""
...
RESULT_DIR = PROJECT_ROOT / "test_ablation" / "base"
TIME_PREFIX = "base_time_res"
FAST_PREFIX = "base_fast_stream_res"
VLM_PREFIX = "base_vlm_res"
TARGET_SAMPLES = 30
...
def initialise_files(
    ...
    started_at = now_text()
    header = (
        "Ablation baseline: full pipeline\n"
        f"run_id: {run_id:03d}\n"
        f"started_at: {started_at}\n"
        f"target_samples: {TARGET_SAMPLES}\n"
        "condition: full pipeline without ablation\n\n"
    )
...
def main() -> int:
    ...
    print("[base] Full pipeline baseline 시작")
    print(f"[base] run_id: {run_id:03d}")
    print(f"[base] time CSV/TXT: {time_csv} | {time_txt}")
    print(f"[base] fast CSV/TXT: {fast_csv} | {fast_txt}")
    print(f"[base] VLM CSV/TXT: {vlm_csv} | {vlm_txt}")
    ...
    try:
        while sample < TARGET_SAMPLES:
            ...
            if frame_count % PROCESS_EVERY_N_FRAMES != 0:
                cv2.imshow("ablation - baseline full pipeline", frame)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    print("[base] 사용자 입력 q로 중단합니다.")
                    return 1
                continue
            ...
            if not inputs:
                print("[base] 입력 객체가 없어 샘플로 집계하지 않습니다.")
                cv2.imshow("ablation - baseline full pipeline", frame)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    print("[base] 사용자 입력 q로 중단합니다.")
                    return 1
            ...
            print(f"[base] sample {next_sample}/{TARGET_SAMPLES} VLM 호출 (객체 {len(inputs)}개)")

            try:
                batch_output = interpreter.interpret_batch(batch_input, image=pil_image)
            except Exception as exc:
                print(f"[base] VLM 호출 실패, 샘플로 집계하지 않습니다: {exc}")
                cv2.imshow("ablation - baseline full pipeline", vlm_frame)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    print("[base] 사용자 입력 q로 중단합니다.")
                    return 1
                continue
            ...
            print(
                f"[base] sample {sample}/{TARGET_SAMPLES} 저장 완료 | "
