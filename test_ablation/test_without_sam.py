"""Ablation: SAM 없는 전체 파이프라인.

SAM만 제거하고, YOLO bbox 기반 coarse centroid/area로 동일한 입력 스키마를 유지한다.
SAM 모델 로드, SAM 마스크 생성, SAM 캐시 재사용은 수행하지 않는다.

생성 파일 3쌍:
1. SAM 없는 전체 파이프라인 시간
   - without_sam_time_res_XXX.csv/txt
   - 전체 파이프라인 시간 30개 + 통계(mean/std/median/Q1/Q3/IQR/outlier)

2. FAST_STREAM 데이터
   - without_sam_fast_stream_res_XXX.csv/txt
   - Vision/Geometry 이후 Unity로 빠르게 보내는 좌표 데이터 30개

3. VLM 데이터
   - without_sam_vlm_res_XXX.csv/txt
   - VLM 추론 결과 JSON 30개
"""
...
RESULT_DIR = PROJECT_ROOT / "test_ablation" / "without_sam"
TIME_PREFIX = "without_sam_time_res"
FAST_PREFIX = "without_sam_fast_stream_res"
VLM_PREFIX = "without_sam_vlm_res"
TARGET_SAMPLES = 30
...
def initialise_files(
    ...
    started_at = now_text()
    header = (
        "Ablation: without SAM full pipeline\n"
        f"run_id: {run_id:03d}\n"
        f"started_at: {started_at}\n"
        f"target_samples: {TARGET_SAMPLES}\n"
        "condition: SAM removed. YOLO bbox coarse centroid/area is used instead of SAM mask.\n\n"
    )
...
def attach_bbox_coarse_sam_fields(scene_data: dict, depth_data: dict) -> dict:
    """SAM을 실행하지 않고 YOLO bbox만으로 downstream 필수 필드를 채운다.

    - obj["sam"]에는 실제 SAM 마스크가 아니라 bbox 기반 coarse centroid/area를 기록한다.
    - depth는 bbox 사각형 ROI에서 robust representative depth를 계산한다.
    """
    import numpy as np

    from vision.depth.depth_estimator import robust_representative_depth

    if not scene_data or not scene_data.get("objects") or depth_data is None:
        return scene_data
    ...
    for obj in scene_data.get("objects", []):
        ...
        if x2 <= x1 or y2 <= y1:
            cx = max(0, min(width - 1, x1))
            cy = max(0, min(height - 1, y1))
            obj["sam"] = {
                "mask_path": None,
                "mask_area": 0,
                "centroid_2d": [cx, cy],
        ...
        cx = int((x1 + x2) / 2)
        cy = int((y1 + y2) / 2)
        roi_pixels = raw_depth[y1:y2, x1:x2]
...
def process_without_sam_frame(frame, frame_count: int, modules: dict) -> tuple[dict, object]:
    """YOLO는 유지하고 SAM만 제거한 파이프라인을 한 프레임 실행한다."""
    yolo_result = modules["detector"].detect(frame)
    scene_data = modules["detector"].build_scene(yolo_result, frame, frame_count)
    depth_map = modules["depth_estimator"].get_depth_map(frame)
    scene_data = attach_bbox_coarse_sam_fields(scene_data, depth_map)
    ...
    return scene_data, yolo_result.plot()
...
def main() -> int:
    ...
    print("[without_sam] SAM 없는 전체 파이프라인 시작")
    print(f"[without_sam] run_id: {run_id:03d}")
    print(f"[without_sam] time CSV/TXT: {time_csv} | {time_txt}")
    print(f"[without_sam] fast CSV/TXT: {fast_csv} | {fast_txt}")
    print(f"[without_sam] VLM CSV/TXT: {vlm_csv} | {vlm_txt}")
    print("[without_sam] SAM 모델은 로드/실행하지 않고 YOLO bbox 기반 coarse centroid/area를 사용합니다.")
    ...
    try:
        while sample < TARGET_SAMPLES:
            ...
            if frame_count % PROCESS_EVERY_N_FRAMES != 0:
                cv2.imshow("ablation - without SAM", frame)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    print("[without_sam] 사용자 입력 q로 중단합니다.")
                    return 1
                continue

            total_started_at = time.perf_counter()
            # base와 CSV 형식을 맞추기 위해 컬럼은 유지하지만,
            # SAM은 실행하지 않으므로 값은 0으로 고정한다.
            sam_frame_count = 0

            scene_data, annotated_frame = process_without_sam_frame(frame, frame_count, modules)
            inputs = build_inputs_from_scene(scene_data)
            if not inputs:
                print("[without_sam] 입력 객체가 없어 샘플로 집계하지 않습니다.")
                cv2.imshow("ablation - without SAM", frame)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    print("[without_sam] 사용자 입력 q로 중단합니다.")
                    return 1
            ...
            batch_input = SemanticInterpretationBatchInput(
                context=(
                    f"{DEFAULT_CONTEXT}. "
                    "Ablation condition: SAM is removed. "
                    "Object centroid and area are coarse estimates derived from YOLO bounding boxes, not SAM masks."
                ),
                objects=inputs,
            )

            next_sample = sample + 1
            print(f"[without_sam] sample {next_sample}/{TARGET_SAMPLES} VLM 호출 (객체 {len(inputs)}개)")

            try:
                batch_output = interpreter.interpret_batch(batch_input, image=pil_image)
            except Exception as exc:
                print(f"[without_sam] VLM 호출 실패, 샘플로 집계하지 않습니다: {exc}")
                cv2.imshow("ablation - without SAM", vlm_frame)
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    print("[without_sam] 사용자 입력 q로 중단합니다.")
                    return 1
                continue
            ...
            print(
                f"[without_sam] sample {sample}/{TARGET_SAMPLES} 저장 완료 | "
        ...
        print("\n[without_sam] KeyboardInterrupt로 중단합니다.")
        return 1
    finally:
        stream.release()
    ...
    print("[without_sam] 30개 샘플 수집 완료")
    print(f"[without_sam] time CSV/TXT: {time_csv} | {time_txt}")
    print(f"[without_sam] fast CSV/TXT: {fast_csv} | {fast_txt}")
    print(f"[without_sam] VLM CSV/TXT: {vlm_csv} | {vlm_txt}")
    return 0
